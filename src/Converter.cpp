    ...
    const int start, const int end, const int verbose)
{
    ...
    for(size_t i = 0; i < dataStreams.size(); i++)
    {
        ...
        Converter conv(streamName, *stream->getType(), packer, verbose);
...
Converter::Converter(std::string const& basename, Typelib::Type const& type, msgpack_packer& pk, int verbose = 0)
    : type(type), pk(pk),
